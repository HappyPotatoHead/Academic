...
def get_splits(df, test_size):
    ...
    X = df.drop(columns=["heart_attack_risk"]).to_numpy()
    y = df["heart_attack_risk"].to_numpy().reshape(-1,1)
...
def xavier_init(layer_sizes, rand_seed = 42):
    ...
    # Calculates each layer separately
    for layer in range(1, L+1):
        current_F = layer_sizes[layer]
        previous_F = layer_sizes[layer-1]
        
        std = np.sqrt(2 / (current_F + previous_F))
        
        # The Xavier initialisation uses normal distribution
        weights[layer] = np.random.normal(0, std, (previous_F, current_F))
        biases[layer] = np.zeros((current_F, 1))
...
def forward (X, weights, biases, act_functions):
    ...
    for i in range(1, L+1):
        A, cache = forward_linear(A, weights[i], biases[i], act_functions[i])
        caches[i] = cache
...
def backprop(y_hat, y_true, act_functions, caches):
    ...
    for i in range(L, 0, -1):
        ...
        dA, dW, db = backprop_linear_layer(dA, cache, act)
...
# ---------------------------------------------------- SECTION 4 ----------------------------------------------------   
def train(X_train, y_train, layer_sizes, act_functions, lr, num_iters, verbose=True):
    """
    ...
    - verbose: Boolean
        Flat to print training progress
    ...
    weights, biases = xavier_init(layer_sizes, 42)

    # repeat until convergence 
    for i in range(num_iters): # Epoch
        ...
        for layer in range(1, L + 1):
            weights[layer] -= lr * weights_grad[layer]
            biases[layer] -= lr * biases_grad[layer]

        # update progress
        if i == 0 or (i+1) % 5 == 0 or i+1 == num_iters:
            history['iter'].append(i)
            history['cost'].append(cost)
            if verbose: print(f'Iter {i+1:3d} : training cost = {cost:.4f}')
...
def evaluate(X, y_true, weights, biases, act_functions):
    ...
    # Forward propagation
    y_hat, _caches = forward(X, weights, biases, act_functions)
